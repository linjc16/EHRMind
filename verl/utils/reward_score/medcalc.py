...
def calculate_answer_score(json_str, target, calid, upper_limit, lower_limit):
    """Calculate answer score based on final_prediction idx."""
    try:
        data = json.loads(json_str)
        answer = str(data.get("answer"))
        answer_score = check_correctness(answer, target, calid, upper_limit, lower_limit)
...
def compute_score(solution_str, ground_truth, data_source, format_reward=0.1):
    ...
    #     print(f"Format score: {format_scosre}")
    ...
    if format_correct and answer_text:
        answer_score = calculate_answer_score(answer_text, target, calid, upper_limit, lower_limit)
