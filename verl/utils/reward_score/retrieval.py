...
from src.Lucene.utils import ndcg_at_k, recall_at_k

index_dir_dict = {
    "PMC-Patients": "data/local_index_search/retrieval/PMC-Patients/pyserini_index",
    'sigir': "data/local_index_search/retrieval/sigir/pyserini_index",
    'trec_2021': "data/local_index_search/retrieval/trec_2021/pyserini_index",
    'trec_2022': "data/local_index_search/retrieval/trec_2022/pyserini_index"
}
try:
    search_system_dict = {
        "PMC-Patients": PyseriniMultiFieldSearch(index_dir=index_dir_dict['PMC-Patients']),
        'sigir': PyseriniMultiFieldSearch(index_dir=index_dir_dict['sigir']),
        'trec_2021': PyseriniMultiFieldSearch(index_dir=index_dir_dict['trec_2021']),
        'trec_2022': PyseriniMultiFieldSearch(index_dir=index_dir_dict['trec_2022'])
    }
...
def check_json_format(json_str, do_print=False):
    ...
    try:
        ...
        #     if do_print:
        #         print("[Error] Query is not a list")
        #     return False
...
def calculate_answer_score(json_str, label, scores, top_k, domain_name, eval_metric_name):
    """Calculate answer score based on final_prediction idx."""
    try:
        data = json.loads(json_str)
        query = data['query']
        target = label
        results = retriver_items([query], domain_name, top_k=top_k, threads=32)
        pred_results = [item[0] for item in results[query]]
        
        # nctid2score = defaultdict(float)

        # for condition_idx, query in enumerate(query_list):
        #     top_nctids = [item[0] for item in results[query]]

        #     for rank, nctid in enumerate(top_nctids):
        #         score = (1 / (rank + 20)) * (1 / (condition_idx + 1))
        #         nctid2score[nctid] += score   

        # pred_results = [nctid for nctid, _ in sorted(nctid2score.items(), key=lambda x: x[1], reverse=True)][:top_k]
        def soft_penalty(num_targets, scale=3, min_penalty=0.6):
            # output in [min_penalty, 1.0]
            decay = math.exp(-num_targets / scale)
            return min_penalty + (1 - min_penalty) * (1 - decay)
        
        if eval_metric_name == 'recall':
            answer_score = recall_at_k(pred_results, target, top_k)
        else:
            answer_score = ndcg_at_k(pred_results, target, top_k, rel_scores=scores)
            answer_score = answer_score * soft_penalty(len(scores))
...
def compute_score(solution_str, ground_truth, data_source, format_reward=0.1):
    ...
    if 'sigir' in data_source:
        domain_name = 'sigir'
    elif 'trec_2021' in data_source:
        domain_name = 'trec_2021'
    elif 'trec_2022' in data_source:
        domain_name = 'trec_2022'
    else:
        domain_name = 'PMC-Patients'
    
    if '_test' in data_source or '_val' in data_source:
        top_k = 100
        eval_metric_name = 'recall'
    else:
        top_k = 500
    ...
    if format_correct and answer_text:
        answer_score = calculate_answer_score(answer_text, label, scores, top_k, domain_name, eval_metric_name)
...
if __name__ == '__main__':
    solution_str = """<|start_header_id|>assistant<|end_header_id|>: <think> </think> <answer>{"summary": "xx", "query": "Microstructural development"}</answer>
"""
    ground_truth = {'target': ['4983'], 'score': [1]}
