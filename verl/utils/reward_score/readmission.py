...
def check_json_format(json_str, do_print=False):
    ...
    try:
        ...
        if label == 0:
            if prediction != "no readmission":
                if do_print:
                    print("[Error] For label 0, prediction must be 'no readmission'")
                return False
        elif label == 1:
            if prediction != "readmission":
                if do_print:
                    print("[Error] For label 1, prediction must be 'readmission'")
                return False
        else:
            if do_print:
...
def compute_score(solution_str, ground_truth, data_source, format_reward=0.1):
    ...
    if format_correct and answer_text:
        answer_score = calculate_answer_score(answer_text, label)
