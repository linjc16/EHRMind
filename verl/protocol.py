...
class DataProto:
    ...
    def from_items(items: List['DataProtoItem']) -> 'DataProto':
        """
        Construct a DataProto from a list of DataProtoItem.

        Args:
            items (List[DataProtoItem]): A list of DataProtoItem.

        Returns:
            DataProto: A combined DataProto.
        """
        if not items:
            return DataProto()

        # Extract all the batches and stack them
        batches = [item.batch for item in items]
        tensor_batch = torch.stack(batches)

        # Combine non_tensor_batch entries
        non_tensor_batch_list = [item.non_tensor_batch for item in items]
        combined_non_tensor_batch = list_of_dict_to_dict_of_list(non_tensor_batch_list)
        for key, val in combined_non_tensor_batch.items():
            combined_non_tensor_batch[key] = np.array(val, dtype=object)

        # Optional: Combine meta_info if needed (not all items might have it)
        # For now, we take meta_info from the first one
        meta_info = items[0].meta_info if hasattr(items[0], "meta_info") else {}

        return DataProto(batch=tensor_batch, non_tensor_batch=combined_non_tensor_batch, meta_info=meta_info)
