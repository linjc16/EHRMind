...
def compute_reward_metrics(batch):
    ...
    ndcg_at_k = (torch.sum(reward_tensor[reward_tensor > format_score] - format_score).float()) / reward_tensor.numel()
    reward_metrics["reward/ndcg"] = ndcg_at_k
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for data_source, rewards in data_source_reward.items():
            ...
            # all_correct = sum(reward > format_score for reward in rewards)
            # metric_dict[f'val/recall/{data_source}'] = all_correct / total_count if total_count > 0 else 0
